...
import (
	...
	"github.com/cloud104/automated-tests/executors/ginkgo/node-local-dns/internal/wire"
)

var _ = Describe("NodeLocalDNS", Ordered, func() {
	var test *wire.Test
	...
	When("the addon is installed", func() {
		It("should have a DaemonSet", func(ctx SpecContext) {
			Eventually(func() int {
				// Fetches the list of NodeLocalDNS DaemonSet
		...
		It("should have a healthy pod in each node", func(ctx SpecContext) {
			Eventually(func() bool {
				...
				// Fetches the list of NodeLocalDNS pods
				labelSelector := metav1.LabelSelector{MatchLabels: map[string]string{
					"k8s-app": "node-local-dns",
				}}
